...
The system now supports configurable body and head detection runs, tuning sweeps, body/head comparison outputs, CSV reports, and dashboard controls for explaining and testing detection settings. The update improves the ability to tune and compare results, but it does not claim final production accuracy because true accuracy requires manually counted or labeled ground truth.
...
The current system compares two approaches:
...
Both modes feed the same analytics flow: tracking, zone occupancy, line crossing, alerts, SQLite logging, CSV export, and Streamlit dashboard comparison.
...
- Body vs head comparison on the same input video.
...
6. Count current occupancy inside configured zones.
7. Count passenger movement across virtual lines.
8. Generate warning or critical alerts when thresholds are crossed.
9. Save analytics events and snapshots to SQLite.
10. Export CSV reports for review.
11. Generate annotated body and head demo videos.
12. Compare body vs head results in the Streamlit dashboard.
13. Tune settings and validate against manual or labeled ground truth.
...
Important body vs head observation: on this specific public sample clip, body detection produced more detections than head detection. This is not an accuracy conclusion. It means body detection may be better suited to this particular sample, while head detection still needs validation and possibly more relevant training data for Indian Railway CCTV.

## Important Note About Accuracy

True accuracy requires ground-truth labeled frames or manually counted frame/time segments.

The current update improves the ability to tune, compare, and inspect body/head detection results. It does not prove final accuracy improvement because the current sample video does not have verified ground truth.
...
4. Run body and head detection against the same segments.
5. Compare body/head counts against manual ground truth.
...
The system now supports accuracy tuning controls such as confidence threshold, image size, IoU, tracker selection, max detections, and optional test-time augmentation. I also added a tuning sweep workflow and generated tuned body/head comparison outputs, including annotated videos and CSV reports.
...
One important point: the current update improves the ability to tune and compare the models, but final accuracy cannot be claimed without ground truth. For true accuracy validation, we need approved Indian Railway CCTV clips and manual or labeled passenger counts for selected frames/time ranges.
...
Sir, I have addressed your feedback. The project now includes accuracy tuning controls, a tuning sweep, body vs head tuned outputs, and clearer documentation for the problem statement, assumptions, scope, out-of-scope items, steps, limitations, and next steps.

Important note: I have not claimed final accuracy because true accuracy needs ground-truth manual counts or labeled Indian Railway CCTV frames. Please share approved CCTV clips, target camera angles, expected accuracy target, zones, and crowd thresholds. Also please confirm whether I should prioritize further training, live CCTV integration, or dashboard polish next.
...
For accuracy, I added tuning controls instead of hardcoding one setting. We can now adjust confidence threshold, image size, IoU, tracker type, maximum detections, and optional test-time augmentation. I also added a tuning sweep script so we can compare settings systematically. The project now generates tuned body detection and head detection outputs for the same sample video.

The current comparison uses two approaches. Body detection uses pretrained YOLO person detection. Head detection uses the Colab-trained `best.pt` model. Head detection is useful when the full body is hidden or far away, but on the current public sample video the body detector produced more detections than the head detector. I am not presenting that as final accuracy because we do not yet have manual ground truth.
